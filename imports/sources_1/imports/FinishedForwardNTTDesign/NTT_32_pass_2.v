    ...
    for(stage = 0; stage < `STAGE_SIZE; stage=stage+1) begin: STAGE_LOOP
        for (block_number = 0; block_number < (1<<stage); block_number=block_number+1) begin: BLOCK_LOOP
                ...
                // we need to calculate 
                    butterfly #(.TWIDDLE(modular_mult(modular_mult(524289,modular_pow(730626,`MODULUS, (`COEF_PER_CLOCK_CYCLE>>(stage+1))) ,`MODULUS),modular_pow(244715,`MODULUS, bit_inverse(block_number)),`MODULUS)))
                    butterfly0(.clk(clk),
