...
wire [`COEF_PER_CLOCK_CYCLE*(`MODULUS_WIDTH)-1:0]  NTT_IN_wire_2;
...
.PSI(modular_pow(`TWIDDLE_2048, `NTT_DIV_BY_RING, `MODULUS)), 
.OMEGA(modular_pow(`TWIDDLE_2048, `NTT_DIV_BY_RING<<1, `MODULUS)), 
...
generate
    ...
    for(ntt_iter = 0; ntt_iter < (1<<(2*`RING_DEPTH-`LOG_N)); ntt_iter=ntt_iter+1) begin: NTT_ITER_LOOP
        ...
        .PSI(1),//`TWIDDLE_2048 is integrated into pointwise multiplication 
        .OMEGA(modular_pow(`TWIDDLE_2048, `COEF_PER_CLOCK_CYCLE<<1, `MODULUS)), 
