...
modular_multiplier modular_multiplier(.clk(clk),.input_a(input_b), .input_b(TWIDDLE[`MODULUS_WIDTH-1:0]), .output_product(mult_output));
assign add_mod_option0 = pipeline_a_0;
assign add_mod_option1 = pipeline_a_0-`MODULUS;
always @(posedge clk) begin
    pipeline_a_0 <= {2'b0, pipeline_a_8} + {2'b0,mult_output};
    pipeline_b_0 <= {2'b0, pipeline_a_8} - {2'b0,mult_output};
